---
title: "Unsupervised Learning : Identifying Multivariate Outliers Using Density-Based Clustering Algorithms"
...
# **1. Table of Contents**
|
| This project explores different density-based clustering algorithms for identifying multivariate outliers using various helpful packages in <mark style="background-color: #CCECFF">**R**</mark>. Methods applied in the analysis to cluster points and detect outliers from high dimensional data included the **Density-Based Spatial Clustering of Applications with Noise**, **Hierarchical Density-Based Spatial Clustering of Applications with Noise**, **Ordering Points to Identify the Clustering Structure**, **Jarvis-Patrick Clustering** and **Shared Nearest Neighbor Clustering** algorithms. The different clustering algorithms were subjectively evaluated based on their capability to effectively capture the latent characteristics between the different resulting clusters. In addition, the values for the outlier detection rate and Rand index obtained for each algorithm were also assessed for an objective comparison of their clustering performance. All results were consolidated in a [<span style="color: #FF0000">**Summary**</span>](#summary) presented at the end of the document.
|
| Density-based clustering is a form of unsupervised learning method aimed at identifying distinctive groups in the data based on a local cluster criterion made up of density-connected points. The algorithms applied in this study (mostly contained in the <mark style="background-color: #CCECFF">**dbscan**</mark> package) attempt to determine potential clusters in a data space composed of a contiguous region of high point density separated from other clusters by contiguous regions of low point density which are typically considered as noise or outliers.
|
##  1.1 Sample Data
|
...
|
| [Density-Based Spatial Clustering of Applications with Noise](https://dl.acm.org/doi/10.5555/3001460.3001507) estimates the density around each data point by counting the number of points in a user-specified epsilon-neighborhood and applies a user-specified minimum points threshold to identify core, border and noise points. Core points are joined into a cluster when they are density-reachable, otherwise they are treated as separate clusters. Border points are assigned to the same cluster their adjacent core points belong to. All points identified as neither a core or border point are treated as noise.
|
...
|
| [Hierarchical Density-Based Spatial Clustering of Applications with Noise](https://link.springer.com/chapter/10.1007/978-3-642-37456-2_14) computes the hierarchical cluster tree representing density estimates along with stability-based flat cluster extraction. Mutual reachability distance values between points are determined and applied as a distance measure to construct a minimum spanning tree. The formulated tree is pruned using a stability-based extraction method to find optimal cuts in the hierarcy representing the resulting clusters.
|
...
|
| [Ordering Points to Identify the Clustering Structure](https://dl.acm.org/doi/10.1145/304181.304187) extracts the clustering structure of a dataset by identifying the density-connected points, and builds a density-based representation of the data by creating an ordered list of points in the form of a reachability plot. Points which are spatially closest become neighbors in the ordering and are grouped together into the same cluster due to similarity in their reachability distances.
|
...
|
| [Jarvis-Patrick Clustering](https://ieeexplore.ieee.org/document/1672233) builds a shared nearest neighbor graph with a user-defined neighborhood size for nearest neighbor sparsification. Any two points are assigned into the same cluster if these are in each other’s nearest neighbor list and share at least a user-defined number threshold of shared nearest neighbors.
|
...
###  1.5.5 Shared Nearest Neighbor Clustering (SNNCLUST)
|
| [Shared Nearest Neighbor Clustering](https://ieeexplore.ieee.org/document/1672233) calculates the number of shared nearest neighbors, the shared nearest neighbor similarity and creates a shared nearest neighbors graph. The number of shared nearest neighbors is the intersection of the neighborhood of two points based on a user-defined size.
|
...
|
| ![](D:/Github_Codes/ProjectPortfolio/Portfolio_Project_25/docs/Project25_Summary.png)
|
# **3. References**
|
...
| **[Article]** [Density-Based Clustering - DBSCAN, OPTICS and DENCLUE](https://www.datamining365.com/2020/04/density-based-clustering-dbscan.html) by Data Mining 365 Team | **[Article]** [Demo of OPTICS Clustering Algorithm](https://scikit-learn.org/stable/auto_examples/cluster/plot_optics.html) by Scikit-Learn Team
