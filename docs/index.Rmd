...
|
| [DBSCAN](https://dl.acm.org/doi/10.5555/3001460.3001507) estimates the density around each data point by counting the number of points in a user-specified epsilon-neighborhood and applies a user-specified minimum points threshold to identify core, border and noise points. Core points are joined into a cluster when they are density-reachable, otherwise they are treated as separate clusters. Border points are assigned to the same cluster their adjacent core points belong to. All points identified as neither a core or border point are treated as noise.
|
...
|
| [JPCLUST](https://ieeexplore.ieee.org/document/1672233) builds a shared nearest neighbor graph with a user-defined neighborhood size for nearest neighbor sparsification. Any two points are assigned into the same cluster if these are in each other’s nearest neighbor list and share at least a user-defined number threshold of shared nearest neighbors.
|
...
|
| [SNNCLUST](https://ieeexplore.ieee.org/document/1672233) calculates the number of shared nearest neighbors, the shared nearest neighbor similarity and creates a shared nearest neighbors graph. The number of shared nearest neighbors is the intersection of the neighborhood of two points based on a user-defined size.
|
